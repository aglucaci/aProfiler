...
class Alignment():
  ...
  def __init__(self, _Alignment, MoleculeType="DNA"):
    ...
    self.NumCodonSites = self._get_numCodonSites(_Alignment, File_Type)
    self.NumSequences = self._get_numSequences(_Alignment, File_Type)
  ...
  def _get_numCodonSites(self, _Alignment, File_Type):
      sites = 0
  ...
  def _get_numSequences(self, _Alignment, File_Type):
      sequences = 0
      with open(_Alignment, "r") as handle:
          for record in SeqIO.parse(handle, File_Type):
              sequences += 1
...
def hello_alignment_profiler():
    print("Hell world for AP")
